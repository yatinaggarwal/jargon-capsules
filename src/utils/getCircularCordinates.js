export const getCircularCords = ({itemsCount=0, canvasWidth=1000, canvasHeight=600}) => {
    ...
    for(let leftCount = 0 ; leftCount < leftSideItemCount; leftCount++){
        ...
        const yPos = direction * (Math.ceil(leftCount/2) * (itemHeight + verticalSpacing));

        const xCord = leftXPos + Math.log10(canvasWidth/itemsCount) * Math.ceil(leftCount/2) * parseInt(leftCount / Math.log10(leftCount+2))
    ...
    for(let rightCount = 0 ; rightCount < rightSideItemsCount; rightCount++){
        ...
        const yPos = direction * Math.ceil(rightCount/2) * (itemHeight + verticalSpacing);
        const xCord = rightXPos - Math.ceil(rightCount/2) * parseInt(rightCount / Math.log10(rightCount+2)) ;
        coordinates.push({x:xCord, y:yPos})
    }
...
export const translateCoordinates = (cords = [], centerCordinate = {x:0, y:0}) => {
    const finalCords = cords.map(cordinate => {
        return {
